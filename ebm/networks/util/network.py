...
class Network():
    ...
    def __init__(self, function,config):
        ...
        self.params = self.function.params()
        self.layers = self.function.layers()
    ...
    def set_input(self, input_values, reset=False):
        ...
        old_batch_size = self.input_layer.state.size(0)
        batch_size = input_values.size(0)
        
        # we set the input tensor on the network's device
        self.input_layer.set_input(input_values.to(self.device))  # FIXME
        ...
        if reset or batch_size != old_batch_size:
            for layer in self._free_layers:
                layer.init_state(batch_size, self.device)  # FIXME